...
class DreamEvalHarness(LM):
    ...
    def generate_until(
        ...
        for batch_idx in range(0, len(requests), self.batch_size):
            ...
            generation_ids, actual_steps = sampler.sample(
            ...
            # Record efficiency metrics for each sample in the batch
            time_per_sample = elapsed_time / len(contexts)
            for i, (ctx, resp) in enumerate(zip(contexts, responses)):
                efficiency_samples.append(
                    {
                        "sample_idx": batch_idx + i,
                        "input": ctx[:500] + "..." if len(ctx) > 500 else ctx,  # Truncate long inputs
                        "output": resp,
                        "time_seconds": time_per_sample,
