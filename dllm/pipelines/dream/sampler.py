...
def compute_spatial_weights(
    ...
    for d in range(1, max_distance + 1):
        kernel[max_distance - d] = geo_decay ** d
        kernel[max_distance + d] = geo_decay ** d
    kernel = kernel.view(1, 1, -1)
    
    unmasked_float = unmasked.float().unsqueeze(1)
    weights = F.conv1d(unmasked_float, kernel, padding=max_distance).squeeze(1)
...
class DreamSampler(BaseSampler):
    ...
    def sample(
        ...
        **kwargs,
        ...
        max_threshold = kwargs.get("max_threshold", config.max_threshold)
        min_threshold = kwargs.get("min_threshold", config.min_threshold)
        geo_decay = kwargs.get("geo_decay", config.geo_decay)
        max_distance = kwargs.get("max_distance", config.max_distance)
        ...
        for i in range(effective_steps):
            ...
            if max_threshold > min_threshold:
                ...
                t_normalized = i / max(total_steps - 1, 1)
                
                spatial_weights = compute_spatial_weights(
                    mask_index, geo_decay, max_distance
                )
                max_spatial = 2 * sum(geo_decay ** d for d in range(1, max_distance + 1))
                spatial_softening = torch.clamp(spatial_weights / max_spatial, 0.0, 1.0)
                
                threshold = max_threshold - (max_threshold - min_threshold) * spatial_softening
            ...
            if combined_mask is not None:
                x0_full = torch.argmax(logits, dim=-1)
                x = torch.where(combined_mask, x0_full, x)
        ...
        if not return_dict:
            return x, actual_step
        else:
            return SamplerOutput(sequences=x, histories=histories, actual_steps=actual_step)
